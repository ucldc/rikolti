...
class CcaVaultRecord(OaiRecord):
    ...
    def map_subject(self) -> Union[list[dict[str, str]], None]:
        ...
        value = self.working_metadata.get('subject')
        if not value:
            return None
    ...
    def map_is_shown_at(self) -> Union[str, None]:
        return self.working_metadata.get("identifier")

    def map_is_shown_by(self) -> Union[str, None]:
        if "type" in self.working_metadata:
            if self.working_metadata.get("type", [])[0].lower() == "image":
                base_url: str = self.working_metadata.get("identifier")
